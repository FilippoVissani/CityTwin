...
/**
 * A message representing a periodic tick event for the ViewActor.
 * This is used to trigger the ViewActor to perform periodic tasks.
 */
 ...
 * @param resources a set containing requested resources
 */
case class AdaptedResourcesStateResponse(resources: Set[Resource]) extends Serializable with ViewActorCommand

object ViewActor :
  def apply(viewName: String, resourcesToCheck: Set[String], width: Int, height: Int): Behavior[ViewActorCommand] =
  ...
  private def viewActorLogic(ctx: ActorContext[ViewActorCommand], view:View, viewName: String,
                             resourceActor: ActorRef[ResourceActorCommand]): Behavior[ViewActorCommand] =
