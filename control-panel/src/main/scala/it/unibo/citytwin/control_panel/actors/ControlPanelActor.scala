...
import it.unibo.citytwin.core.actors.{AskAllResourcesToMainstay, AskMainstaysState, AskResourcesToMainstay, MainstayActorCommand, MainstaysStateResponse, ResourceActor, ResourceActorCommand, ResourcesFromMainstayResponse}
...
case class AdaptedMainstaysStateResponse(mainstays: Set[ActorRef[MainstayActorCommand]]) extends ControlPanelActorCommand with Serializable
        ...
        Behaviors.receiveMessage {
          ...
          case Tick => {
            ctx.ask(resourceActor, ref => AskMainstaysState(ref)) {
              case Success(MainstaysStateResponse(mainstays: Set[ActorRef[MainstayActorCommand]])) => AdaptedMainstaysStateResponse(mainstays)
