...
@main def main(args: String*): ActorSystem[ControlPanelActorCommand] =
  ...
  val citySize: (Double, Double) =
    if args.isEmpty then (0, 0) else (args(1).toDouble, args(2).toDouble)
  ...
  startup(port)(
    ControlPanelActor(citySize, cityMap, persistenceServiceHost, persistenceServicePort)
  )
