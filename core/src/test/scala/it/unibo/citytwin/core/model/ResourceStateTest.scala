...
class ResourceStateTest extends AnyWordSpec with Matchers:
  "Resource" should {
    "Merge correctly another resource" in {
      val resource1 =
        ResourceState(name = Some("sensor1"), nodeState = Some(true), position = Some(Point2D(0, 0)))
      val resource2 =
        ResourceState(name = Some("sensor1"), resourceType = Set(Act), position = Some(Point2D(1, 1)))
