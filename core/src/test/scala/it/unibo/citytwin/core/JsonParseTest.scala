...
class JsonParseTest extends AnyWordSpec with Matchers:
  ...
  "Parser" should {
    "Parse correctly mainstays history JSON string" in {
      ...
      JSONDeSerializer.jsonToMainstaysHistory(jsonString).map(s => s.state) shouldBe List(
        true,
        true,
        true,
        true
      )
      JSONDeSerializer.jsonToMainstaysHistory(jsonString).map(s => s.address) shouldBe List(
        "akka://ClusterSystem@127.0.0.1:2551/user",
        "akka://ClusterSystem@127.0.0.1:2551/user",
        "akka://ClusterSystem@127.0.0.1:2551/user",
        "akka://ClusterSystem@127.0.0.1:2551/user"
      )
      JSONDeSerializer.jsonToMainstaysHistory(jsonString).map(s => s.time.toString) shouldBe List(
        "2023-08-24T18:09:17.581426074",
        "2023-08-24T18:09:17.996343216",
        "2023-08-24T18:09:20.073069294",
        "2023-08-24T18:09:21.739939386"
      )
    }
  }
