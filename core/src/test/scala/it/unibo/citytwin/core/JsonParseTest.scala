...
class JsonParseTest extends AnyWordSpec with Matchers:
  ...
  "Parser" should {
    "Parse correctly mainstays history JSON string" in {
      ...
      JSONParser.jsonToMainstaysHistory(jsonString).map(s => s.state) shouldBe List(
        true,
        true,
        true,
        true
      )
      JSONParser.jsonToMainstaysHistory(jsonString).map(s => s.address) shouldBe List(
        "akka://ClusterSystem@127.0.0.1:2551/user",
        "akka://ClusterSystem@127.0.0.1:2551/user",
        "akka://ClusterSystem@127.0.0.1:2551/user",
        "akka://ClusterSystem@127.0.0.1:2551/user"
      )
      JSONParser.jsonToMainstaysHistory(jsonString).map(s => s.time.toString) shouldBe List(
